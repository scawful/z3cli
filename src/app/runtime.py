...
from core.tool_adapters import ADAPTER_REGISTRY
from app.tooling import build_capability_bridges
...
SPECIALIST_NAMES = ("din", "navi", "nayru")
...
_SERVER_HINTS: dict[str, str] = {
    "yaze-debugger": "navi",
    "yaze-editor": "din",
    "hyrule-historian": "nayru",
    "book-of-mudora": "nayru",
    "afs": "nayru",
    "z3lsp": "din",
    "mesen": "navi",
}
_ADAPTER_PROFILE_MODEL_ALIASES: dict[str, str] = {
    "farore": "navi",
}
    ...
    *,
    ...
    for call in plan_oracle_context_prefetch(prompt)[:max(0, max_calls)]:
        if call.tool_name not in available_tools:
            if call.tool_name != "register_doc":
                continue
            contexts.append({
                ...
                "label": call.label,
                "content": (
                    f"Error: Oracle grounding tool `{call.tool_name}` is not available "
                    "on the current tool bridge."
                ),
            })
            continue
        try:
            ...
            contexts.append({
                ...
                "label": call.label,
                "content": f"Error: Oracle grounding tool `{call.tool_name}` failed: {exc}",
            })
            continue
        text = str(raw or "").strip()
        if not text:
            text = f"Error: Oracle grounding tool `{call.tool_name}` returned no content."
        if call.tool_name != "register_doc" and text.lower().startswith("error:"):
            continue
...
def build_oracle_prefetch_forced_reply(
    ...
    is_unknown = "unknown register" in content_lower
    is_failed = content_lower.startswith("error:")
    if not is_unknown and not is_failed:
        return ""
    ...
    if is_failed:
        return (
            "I couldn't retrieve authoritative `register_doc` evidence for that register from "
            "the current tool bridge. I don't want to answer from memory, so I can't say what "
            "it does from evidence here."
        )
    return (
...
def _tool_hint(prompt: str, models: dict[str, ModelConfig]) -> str | None:
    ...
    for tool_name, profile in tool_map.items():
        ...
        if tool_name in prompt_lower or tool_name.replace("_", " ") in prompt_lower:
            aliased_profile = _ADAPTER_PROFILE_MODEL_ALIASES.get(profile, "")
            if aliased_profile in models:
...
def prompt_prefers_oracle_reasoner(prompt: str) -> bool:
    prompt_lower = prompt.lower()
    return any(keyword in prompt_lower for keyword in ORACLE_REASONER_KEYWORDS)


def _first_spawnable_model_name(
    parent_model: str,
    models: dict[str, ModelConfig],
    candidates: tuple[str, ...],
) -> str:
    for name in candidates:
        worker = models.get(name)
        if worker is not None and can_spawn_model(parent_model, worker):
            return name
    return ""


def _preferred_oracle_coder_name(
    model: ModelConfig,
    models: dict[str, ModelConfig],
) -> str:
    if _normalize_profile_name(model.name) == "oracle-fast":
        candidates = ORACLE_FAST_CODER_CANDIDATES
    else:
        candidates = ORACLE_CODER_CANDIDATES
    return _first_spawnable_model_name(model.name, models, candidates)
...
def build_oracle_coder_prompt(
    ...
    if not _is_oracle_family_model(model):
        return ""
    coder_name = _preferred_oracle_coder_name(model, models)
    reasoner_name = _first_spawnable_model_name(
        model.name,
        models,
        (ORACLE_REASONER_27B_ALIAS,),
    )
    if not coder_name and not reasoner_name:
        return ""

    authoring_request = prompt_prefers_oracle_coder(prompt)
    reasoner_request = prompt_prefers_oracle_reasoner(prompt)
    lines = []
    if coder_name:
        if authoring_request:
            lines.append(
                f"This request is code-authoring work. Delegate the code-writing pass to `spawn_subagent` with model `{coder_name}` unless the user explicitly asked for analysis only.",
            )
        else:
            lines.append(
                f"When a request turns into concrete code authoring, repair, hook writing, refactoring, infill, or repo-grounded patch synthesis, prefer delegating the authoring pass to `spawn_subagent` with model `{coder_name}`.",
            )
        lines.extend([
            "Use tools yourself first when you still need current files, symbols, ROM context, or emulator evidence.",
            f"When you delegate to `{coder_name}`, give it a self-contained brief with the concrete files, constraints, ABI or width expectations, and the exact patch or replacement you want back.",
            f"After `{coder_name}` returns, verify or sanity-check the candidate patch before giving the final answer.",
        ])
        if _normalize_profile_name(model.name) == "oracle-fast":
            lines.append(
                f"Quality-first policy: for real code changes, prefer `{coder_name}` even when you could draft the patch yourself.",
            )
    if reasoner_name:
        if reasoner_request:
            lines.append(
                f"This request benefits from broad model, dataset, architecture, or long-context reasoning. Delegate a bounded analysis pass to `spawn_subagent` with model `{reasoner_name}` unless the user explicitly asked for a direct answer only.",
            )
        else:
            lines.append(
                f"For repo-scale architecture, model-training strategy, dataset review, catalog design, long-context synthesis, or high-uncertainty planning, delegate a bounded analysis pass to `spawn_subagent` with model `{reasoner_name}`.",
            )
        if coder_name:
            lines.append(
                f"Use `{reasoner_name}` for analysis and planning, not final patch authorship; use `{coder_name}` for concrete patches.",
            )
        else:
            lines.append(
                f"Use `{reasoner_name}` for analysis and planning, not final patch authorship.",
            )
    lines.append(
