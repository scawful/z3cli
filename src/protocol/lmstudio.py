...
def _hostd_fallback_allowed(exc: Exception) -> bool:
    detail = str(exc).lower()
    return (
        "afs-hostd request failed:" in detail
        and any(
            hint in detail
            for hint in (
                "connection refused",
...
def _api_request_json(host: str, port: int, path: str, *, timeout: float = DEFAULT_LMS_TIMEOUT_S) -> Any:
    url = f"{_api_base_url(host, port).rstrip('/')}/{path.lstrip('/')}"
...
async def _api_request_json_async(host: str, port: int, path: str, *, timeout: float = DEFAULT_LMS_TIMEOUT_S) -> Any:
    return await asyncio.to_thread(_api_request_json, host, port, path, timeout=timeout)


def _loaded_entries_from_api(host: str, port: int) -> list[dict[str, Any]]:
    try:
        payload = _api_request_json(host, port, "/v1/models")
    except Exception:
        return []
    ...
    for item in data:
        ...
        entries.append({
            "identifier": model_id,
            "modelKey": model_id,
            "displayName": item.get("owned_by") or model_id,
        })
    return entries


async def _loaded_entries_from_api_async(host: str, port: int) -> list[dict[str, Any]]:
    try:
        payload = await _api_request_json_async(host, port, "/v1/models")
    except Exception:
        return []
    data = payload.get("data")
    if not isinstance(data, list):
...
def _can_run_cli_fallback() -> bool:
    return _remote_enabled() or bool(shutil.which("lms"))


def _api_endpoint_running(host: str, port: int) -> bool:
    try:
        _api_request_json(host, port, "/v1/models")
    except Exception:
        return False
    return True


async def _api_endpoint_running_async(host: str, port: int) -> bool:
    try:
        await _api_request_json_async(host, port, "/v1/models")
    except Exception:
        return False
    return True
...
def server_status(host: str, port: int) -> dict[str, Any]:
    if _hostd_enabled():
        try:
            ...
            if not _hostd_fallback_allowed(exc):
                raise
            if _api_endpoint_running(host, port):
                return {"running": True, "port": port}
            if not _can_run_cli_fallback():
                return {}
...
async def server_status_async(host: str, port: int) -> dict[str, Any]:
    if _hostd_enabled():
        try:
            ...
            if not _hostd_fallback_allowed(exc):
                raise
            if await _api_endpoint_running_async(host, port):
                return {"running": True, "port": port}
            if not _can_run_cli_fallback():
                return {}
...
def available_models(host: str, port: int) -> list[dict[str, Any]]:
    if _hostd_enabled():
        try:
            ...
            if not _hostd_fallback_allowed(exc):
                raise
            loaded = _loaded_entries_from_api(host, port)
            if loaded or not _can_run_cli_fallback():
                return loaded
...
async def available_models_async(host: str, port: int) -> list[dict[str, Any]]:
    if _hostd_enabled():
        try:
            ...
            if not _hostd_fallback_allowed(exc):
                raise
            loaded = await _loaded_entries_from_api_async(host, port)
            if loaded or not _can_run_cli_fallback():
                return loaded
...
def loaded_request_name(alias: str, model_id: str, loaded: list[dict[str, Any]]) -> str | None:
    for entry in loaded:
        ...
        for key in loaded_model_lookup_keys(entry):
            if key == alias:
                return identifier if isinstance(identifier, str) and identifier else key
            if key == model_id:
                return identifier if isinstance(identifier, str) and identifier else model_id
    return None
...
def ensure_model_loaded(
    ...
    if _hostd_enabled():
        try:
            ...
            if not _can_run_cli_fallback():
                raise RuntimeError(
                    f"afs-hostd is unavailable, and model '{alias}' is not already loaded at {host}:{port}. "
                    "Load it in LM Studio or restore the hostd tunnel."
                ) from exc
    loaded = loaded_models(host, port)
    request_name = loaded_request_name(alias, model_id, loaded)
    if request_name:
