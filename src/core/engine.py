...
from core.compaction import ConversationCompactor
...
class ChatEngine:
    ...
    async def chat(
        ...
        """
        # ``use_tools`` controls native provider tool schemas. Some local
        # Qwen lanes use manual XML calls instead, so execution permission is a
        # separate gate.
        allow_tool_execution = use_tools if allow_manual_tool_calls is None else allow_manual_tool_calls
        base_system = system
        ...
        for round_idx in range(max_rounds + 1):
            ...
            tool_calls = output.tool_calls if allow_tool_execution else []
            if allow_tool_execution and not tool_calls and output.content:
