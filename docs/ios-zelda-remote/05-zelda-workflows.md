...
- **Slash commands** — `ZeldaWorkflows.slashHints`: only commands that exist in [`src/app/serve.py`](../../src/app/serve.py) (`handle_command`), e.g. `/sessions`, `/status`.
...
The Python side persists sessions under the host user’s data dir (`src/core/session.py`); the phone never reads those files directly.
